...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(docker run --rm "${TOOLS_IMAGE}" peer version | sed -ne 's/ Version: //p' | head -1)
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    CA_DOCKER_IMAGE_VERSION=$(docker run --rm "${CA_IMAGE}" fabric-ca-client version | sed -ne 's/ Version: //p' | head -1)
